...
-->
  ...
  <!-- Set this if you want a local version number -->
  <xsl:param name="keep.numbered.sections" select="'0'"/>
  ...
  <xsl:template match="appendix|article|book|chapter|bibliography|glossary|glossdiv|part|preface|reference|set">
    ...
    <xsl:message># <xsl:value-of select="concat(local-name(), '=>',
      count($info))"/>
      title.outside.info=<xsl:value-of select="$title.outside.info"/>
      title.inside.info=<xsl:value-of select="$title.inside.info"/>
    </xsl:message>
    -->

    <xsl:element name="{local-name()}" namespace="http://docbook.org/ns/docbook">
      <xsl:call-template name="copy.attributes"/>
      <xsl:apply-templates select="processing-instruction()" mode="copy"/>
      <xsl:choose>
        <xsl:when test="$title.inside.info = 1 and $title.outside.info = 0">
<!--          <xsl:message>Found title|subtitle|titleabbre inside *info</xsl:message>-->
          <xsl:apply-templates select="$info/title|$info/subtitle|$info/titleabbrev" mode="copy"/>
        </xsl:when>
        <xsl:when test="$title.inside.info = 0 and $title.outside.info = 1">
<!--          <xsl:message>Found title|subtitle|titleabbre</xsl:message>-->
      ...
      <xsl:apply-templates select="node()[not(self::processing-instruction())]" mode="structure"/>
    </xsl:element>
  </xsl:template>
  ...
  <xsl:template match="bookinfo|setinfo" priority="200" mode="old">
    ...
    <xsl:variable name="title.outside.info">
      <xsl:choose>
        <xsl:when
          test="preceding-sibling::title or preceding-sibling::subtitle or preceding-sibling::titleabbrev">
          <xsl:text>1</xsl:text>
        </xsl:when>
        <xsl:otherwise>
          <xsl:text>0</xsl:text>
    ...
    <info>
      <xsl:if test="$title.inside.info = '1' and $title.outside.info = '1'">
        ...
        <xsl:if test="preceding-sibling::subtitle and not(./subtitle)">
          <xsl:apply-templates select="preceding-sibling::subtitle"
            mode="copy"/>
        </xsl:if>
        <xsl:if
          test="preceding-sibling::titleabbrev and not(./titleabbrev)">
  ...
  <xsl:template match="blockinfo|indexinfo|
    ...
    <xsl:choose>
      <xsl:when test="$title.inside.info = '0'">
        <xsl:call-template name="emit-message">
          <xsl:with-param name="message">
            <xsl:text>Keeping one or more title elements before </xsl:text>
            <xsl:value-of select="local-name(.)"/>
          </xsl:with-param>
        </xsl:call-template>
      ...
      <xsl:otherwise>
        <xsl:call-template name="emit-message">
          <xsl:with-param name="message">
            <xsl:text>Moving one or more title elements into </xsl:text>
            <xsl:value-of select="local-name(.)"/>
          </xsl:with-param>
        </xsl:call-template>
        <info>
  ...
  <xsl:template match="refentryinfo" priority="200">
    <info>
      <xsl:call-template name="copy.attributes"/>
  ...
  <xsl:template match="refmiscinfo" priority="200">
    <refmiscinfo>
      <xsl:call-template name="copy.attributes">
  ...
  <xsl:template match="author[not(personname)]|editor[not(personname)]|othercredit[not(personname)]"
    ...
    <xsl:copy>
      ...
      <personname>
        <xsl:apply-templates
          select="honorific|firstname|surname|othername|lineage"/>
      </personname>
      <xsl:apply-templates
        select="*[not(self::honorific|self::firstname|self::surname
    ...
    <xsl:copy>
      <xsl:call-template name="copy.attributes">
        <xsl:with-param name="suppress.default"
          select="'inheritnum=ignore continuation=restarts'"/>
      </xsl:call-template>
      <xsl:apply-templates/>
    </xsl:copy>
  </xsl:template>
  ...
  <xsl:template match="equation" priority="200">
    <xsl:choose>
      <xsl:when test="not(title)">
        <xsl:call-template name="emit-message">
          <xsl:with-param name="message">Convert equation without title
            to informal equation.</xsl:with-param>
        </xsl:call-template>
        <informalequation>
          <xsl:call-template name="copy.attributes"/>
          <xsl:apply-templates/>
  ...
  <xsl:template match="contractnum" priority="200">
    <xsl:if
      test="not(preceding-sibling::contractsponsor
  ...
  <xsl:template match="biblioentry/firstname
    ...
    <xsl:choose>
      <xsl:when
        test="preceding-sibling::firstname
  ...
  <xsl:template match="date|pubdate" priority="200">
    ...
    <xsl:variable name="date">
      <xsl:choose>
        <xsl:when
          test="string($p1+1) != 'NaN' and string($p3+1) != 'NaN'">
          <xsl:choose>
        ...
        <xsl:when
          test="string($p2+1) != 'NaN' and string($p3+1) != 'NaN'">
          <xsl:choose>
  ...
  <xsl:template match="title|subtitle|titleabbrev" priority="300" mode="old">
    ...
    <xsl:choose>
      <xsl:when
        test="../*[local-name(.) = concat($parent.name, 'info')]/*[local-name(.) = $local.name]">
        <xsl:call-template name="emit-message">
          <xsl:with-param name="message">
      ...
      <xsl:when
        test="$parent.name = 'article' and ../artheader/*[local-name(.) = $local.name]">
        <xsl:call-template name="emit-message">
          <xsl:with-param name="message">
            <xsl:text>Check </xsl:text>
            <xsl:value-of select="$parent.name"/>
      ...
      <xsl:when
        test="preceding-sibling::*[local-name(.) = concat($parent.name, 'info')]">
        <xsl:call-template name="emit-message">
          <xsl:with-param name="message">
            ...
            <xsl:value-of select="$parent.name"/>
            <xsl:text>info. Moved before or inside info.</xsl:text>
          </xsl:with-param>
        </xsl:call-template>
      </xsl:when>
      ...
      <xsl:when
        test="following-sibling::bookinfo[title|subtitle|titleabbrev] or
  ...
  <xsl:template match="abstract" priority="300">
    <xsl:choose>
      <xsl:when
        test="not(contains(name(parent::*),'info'))
...
-->
  ...
  <xsl:template match="classsynopsis|cmdsynopsis|constructorsynopsis
    ...
    <xsl:call-template name="emit-message">
      <xsl:with-param name="message">
        <xsl:text>CHECK OUTPUT: Found </xsl:text>
        <xsl:value-of select="local-name(.)"/>
  ...
  <xsl:template
    match="authorinitials|corpcredit|interface|medialabel|othercredit|revhistory"
    mode="clean-terms">
    <xsl:call-template name="emit-message">
      <xsl:with-param name="message">
  ...
  <xsl:template
    match="affiliation[not(parent::author) and not(parent::collab) and not(parent::editor) and not(parent::org) and not(parent::othercredit) and not(parent::person)]">
    <xsl:call-template name="emit-message">
      <xsl:with-param name="message">
        <xsl:text>CHECK OUTPUT: Converting </xsl:text>
        <xsl:value-of select="local-name(.)"/>
        <xsl:text> to phrase in </xsl:text>
        <xsl:value-of select="local-name(..)"/>
      </xsl:with-param>
    </xsl:call-template>
    <phrase remap="{local-name(.)}">
  ...
  <xsl:template name="copy.attributes">
    ...
    <xsl:for-each select="$src/@*">
      <xsl:variable name="suppressed.value">
        <xsl:choose>
          <xsl:when
            test="not(contains($suppress.default, concat(local-name(.),'=')))">
            <xsl:text>this-value-never-matches</xsl:text>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of
              select="substring-before(substring-after(concat($suppress.default,' '), concat(local-name(.),'=')),' ')"
            />
          </xsl:otherwise>
        </xsl:choose>
      </xsl:variable>
      ...
      <xsl:choose>
        ...
        <xsl:when
          test="local-name($src) = 'simplemsgentry'
        ...
        <xsl:when
          test="local-name($src) = 'refmiscinfo'
                      and local-name(.) = 'role'
                      and . = 'type'
                      ">
          <xsl:call-template name="emit-message">
  ...
  <xsl:template match="*" mode="addNS" name="addNS">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:copy>
          <xsl:if
            test="namespace-uri(.) = 'http://docbook.org/ns/docbook' and
